...
public interface AlertsRepository extends JpaRepository<Alerts, UID> {
    ...
    @Query("SELECT a FROM Alerts a WHERE a.tenantId like %?1 AND a.tenantId like %?2 AND DAY(a.alertDate)=4 AND a.alertDate BETWEEN ?3 AND ?4 order by a.alertDate DESC")
