...
public class VesselTrackingItemController {

    private static final Logger logger = LoggerFactory.getLogger(VesselTrackingItemController.class);
    @Autowired
    private VesselTrackingItemsRepository vesselTrackingItemRepository;
    ...
    public ResponseEntity getBrokenRule(@RequestParam(value = "tenantId1", required = false) String tenantId1, @RequestParam(value = "tenantId2", required = false) String tenantId2, @RequestParam(value = "dateStart") String startDate, @RequestParam(value = "dateEnd") String endDate, HttpServletResponse response)
        ...
        logger.info(endDate);
        Collection<Alerts> brokenRules = vesselTrackingItemRepository
